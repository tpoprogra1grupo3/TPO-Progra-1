...
def crear_prestamos(usuarios_datos, libros, prestamos):
    while True:
        ...
        if len(libro_a_alquilar) > 0 and len(editorial) > 0:
            fila_libro = buscar_libro(libros, libro_a_alquilar, editorial)
            if fila_libro is not None:
                        ...
                        if semanas_a_alquilar.isnumeric() and semanas_a_alquilar != "0":
                            ...
                            while True:
                                ...
                                if usuario_que_alquila.isalpha():
                                    existe_usuario = buscar_nombre_usuario(usuarios_datos, usuario_que_alquila)
                                    if existe_usuario is not None:
                                        id_usuario_que_alquila = usuarios_datos[usuario_que_alquila]['id']
                                        ...
                                        print(f"{usuario_que_alquila} | {id_usuario_que_alquila} | {libro_a_alquilar} | {id_libro_a_alquilar} | {autor_del_libro} | {editorial} | ${precio:.2f} | {estado_prestamo(fecha_de_vencimiento)}")
                                        ...
                                        while True:
                                            ...
                                            if opcion == "1":
                                                ...
                                                libros[fila_libro][3] -= 1  # Resta 1 unidad al stock
                                                limpiar_consola()
                            ...
                            print("La cantidad de semanas es inválida")
                else:
                    print("No hay más unidades disponibles de ese libro")
            else:
                print("El libro o editorial no existen")
        else:
            print("El libro o editorial ingresados son inválidos")
